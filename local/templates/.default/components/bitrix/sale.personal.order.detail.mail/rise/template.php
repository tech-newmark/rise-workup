...
/** @var array $arResult */
?>
<div class="bx_order_list" style="width: 100%; max-width: 600px;">
	<?php
					...
					<? if ($arParams["SHOW_ORDER_PAYMENT"] == 'Y'): ?>
						...
						foreach ($arResult["PAYMENT"] as $payment) {
							...
							$titleParams = [
								...
								"#DATE_BILL#" => $payment['DATE_BILL_FORMATED'],
							];
						?>
							<tr>
			...
			<table class="bx_order_list_table_order" style="width:100%; min-width: 0; border-collapse: collapse;">
				<thead>
					<tr>
							...
							if ($headerName == '') {
								...
								foreach (array_values($arResult['PROPERTY_DESCRIPTION']) as $prop_head_desc):
									if (array_key_exists($headerId, $prop_head_desc)) {
										...
										if ($headerId === 'PROPERTY_COLOR2_REF') {
											$headerName = 'Допцвет';
										}
									}
						...
						?>
							<td style="border: 1px solid #000; padding: 5px" <?= $colspan ?>><?= $headerName ?></td><?
																																																		endforeach;
																																																			?>
					</tr>
				</thead>
				<tbody>
					...
					?>
						<tr><?
								...
								?>
								<td class="custom" style="border: 1px solid #000; padding: 5px"><?
																																																																																		...
																																																																																		elseif ($headerId == "PICTURE"):
																																																																																			...
																																																																																			if ($hasLink):
																																																																																				?><a href="<?= $prod["DETAIL_PAGE_URL"] ?>" target="_blank"><?
																																																																																																																	...
																																																																																																																	if ($prod['PICTURE']['SRC']):
																																																																																																																		?><img src="<?= $prod['PICTURE']['SRC'] ?>" width="40"
													height="40" alt="<?= $prod['NAME'] ?>" /><?
																																																																																																																	endif;
																																																																																																																	if ($hasLink):
																																		?></a><?
																																																																																																																	endif;
											...
											<? foreach ($prod["PROPS"] as $prop): ?>
												<tr>
													...
													<td style="border: 1px solid #000; padding: 5px">
														<b><?= htmlspecialcharsbx($prop["VALUE"]) ?></b>
													</td>
												</tr>
											<? endforeach ?>
										</table>
			...
			<table class="bx_ordercart_order_sum" style="width:100%; min-width: 0; border-collapse: collapse;">
				<tbody>
					...
					<? if (floatval($arResult["ORDER_WEIGHT"])): ?>
						<tr>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t1"><?= GetMessage('SPOD_TOTAL_WEIGHT') ?>:</td>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t2"><?= $arResult['ORDER_WEIGHT_FORMATED'] ?></td>
						</tr>
					<? endif ?>
					...
					?>
					<tr>
						<td style="border: 1px solid #000; padding: 5px" class="custom_t1"><?= GetMessage('SPOD_PRODUCT_SUM') ?>:</td>
						<td style="border: 1px solid #000; padding: 5px" class="custom_t2"><?= $arResult['PRODUCT_SUM_FORMATED'] ?></td>
					</tr>
					...
					<? if ($arResult["PRICE_DELIVERY_FORMATED"] <> ''): ?>
						<tr>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t1"><?= GetMessage('SPOD_DELIVERY') ?>:</td>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t2"><?= $arResult["PRICE_DELIVERY_FORMATED"] ?></td>
						</tr>
					<? endif ?>
					...
					<? foreach ($arResult["TAX_LIST"] as $tax): ?>
						<tr>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t1"><?= $tax["TAX_NAME"] ?>:</td>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t2"><?= $tax["VALUE_MONEY_FORMATED"] ?></td>
						</tr>
					<? endforeach ?>
					...
					<? if (floatval($arResult["TAX_VALUE"])): ?>
						<tr>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t1"><?= GetMessage('SPOD_TAX') ?>:</td>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t2"><?= $arResult["TAX_VALUE_FORMATED"] ?></td>
						</tr>
					<? endif ?>
					...
					<? if (floatval($arResult["DISCOUNT_VALUE"])): ?>
						<tr>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t1"><?= GetMessage('SPOD_DISCOUNT') ?>:</td>
							<td style="border: 1px solid #000; padding: 5px" class="custom_t2"><?= $arResult["DISCOUNT_VALUE_FORMATED"] ?></td>
						</tr>
					<? endif ?>

					<tr>
						<td style="border: 1px solid #000; padding: 5px" class="custom_t1 fwb"><?= GetMessage('SPOD_SUMMARY') ?>:</td>
						<td style="border: 1px solid #000; padding: 5px" class="custom_t2 fwb"><?= $arResult["PRICE_FORMATED"] ?></td>
					</tr>
				</tbody>
			</table>
		<? endif ?>
