(function (window) {
	...
	window.JCCatalogCompareList.prototype.reloadResult = function (result) {
		...
		if (BX.type.isNotEmptyString(result)) {
			if (result.indexOf("<table") >= 0) mode = "block";
		}
	...
	window.JCCatalogCompareList.prototype.deleteCompareResult = function (
		...
		if (BX.type.isPlainObject(result)) {
			if (
				BX.type.isNotEmptyString(result.STATUS) &&
				result.STATUS === "OK" &&
				!!result.ID
			) {
				...
				if (BX.type.isElementNode(tbl)) {
					if (tbl.rows.length > 1) {
						...
						if (
							BX.type.isNotEmptyString(result.COUNT) ||
							BX.type.isNumber(result.COUNT)
						) {
