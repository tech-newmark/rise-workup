(function () {
	...
	BX.Sale.BasketFilter.prototype.onFilterInput = function () {
		var value = BX.type.isDomNode(BX.proxy_context)
			? BX.util.trim(BX.proxy_context.value).toLowerCase()
			: "";
	...
	BX.Sale.BasketFilter.prototype.clearAllFiltersExcept = function (names) {
		...
		!BX.util.in_array("not-available", names) &&
			this.clearNotAvailableItemsFilter();
	...
	BX.Sale.BasketFilter.prototype.onFilterChange = function () {
		...
		if (
			...
			this.filterTimer = setTimeout(
				BX.proxy(this.enableFilterMode, this),
	...
	BX.Sale.BasketFilter.prototype.enableFilterMode = function () {
		...
		if (this.lastShownItemsHash !== foundItemsHash) {
			...
			this.component.deleteBasketItems(
				BX.util.array_values(this.component.shownItems),
				false,
			);
		...
		for (var i in this.component.shownItems) {
			if (this.component.shownItems.hasOwnProperty(i)) {
				this.highlightSearchMatch(
					this.component.items[this.component.shownItems[i]],
				);
			}
		}
	};
	...
	BX.Sale.BasketFilter.prototype.searchItemMatch = function (item) {
		...
		if (BX.type.isNotEmptyString(this.currentFilter.query)) {
			...
			if (!match && this.currentFilter.query.length >= 3) {
				if (
					item.PRICE_FORMATED.toLowerCase().indexOf(
						this.currentFilter.query,
					) !== -1
				) {
					match = "PRICE";
				} else if (
					item.SUM_PRICE_FORMATED.toLowerCase().indexOf(
						this.currentFilter.query,
					) !== -1
				) {
					match = "SUM_PRICE";
			...
			if (!match && item.PROPS.length) {
				for (k in item.PROPS) {
					if (
						item.PROPS.hasOwnProperty(k) &&
						BX.type.isNotEmptyString(item.PROPS[k].VALUE)
					) {
						lcValue = item.PROPS[k].VALUE.toLowerCase();

						if (
							lcValue === this.currentFilter.query ||
							(this.currentFilter.query.length >= 3 &&
								lcValue.indexOf(this.currentFilter.query) !== -1)
						) {
							match = "PROPS";
							break;
						}
			...
			if (!match && item.COLUMN_LIST.length) {
				for (k in item.COLUMN_LIST) {
					if (
						item.COLUMN_LIST.hasOwnProperty(k) &&
						BX.type.isNotEmptyString(item.COLUMN_LIST[k].VALUE)
					) {
				...
				case "NAME":
					...
					if (BX.type.isDomNode(entity)) {
						entity.innerHTML = itemData.NAME.replace(
							new RegExp(
								"(.*)(" +
									this.currentFilter.query.replace(
										/[.*+?^${}()|[\]\\]/g,
										"\\$&",
									) +
									")(.*)",
				...
				case "PROPS":
					...
					for (i = 0; i < entity.length; i++) {
						...
						for (k in itemData.PROPS) {
							if (
								itemData.PROPS.hasOwnProperty(k) &&
								itemData.PROPS[k].CODE === code
							) {
				...
				case "COLUMNS":
					...
					for (i = 0; i < entity.length; i++) {
						...
						for (k in itemData.COLUMN_LIST) {
							if (
								itemData.COLUMN_LIST.hasOwnProperty(k) &&
								itemData.COLUMN_LIST[k].CODE === code
							) {
	...
	// 		var emptyResultNode = this.component.getCacheNode(
	// 			this.component.ids.itemListEmptyResult,
	// 		);
