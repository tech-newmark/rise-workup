...
if ($this->hideCoupon !== 'Y' && !empty($result['COUPON_LIST'])) {
	...
	foreach ($totalData['COUPON_LIST'] as &$coupon) {
		if ($coupon['JS_STATUS'] === 'ENTERED') {
			$coupon['CLASS'] = 'warning';
			$coupon['STATUS_TEXT'] = 'не может быть использован';
			$coupon['CHECK_CODE_TEXT'][0] = 'не может быть использован';
			$coupon['JS_CHECK_CODE'] = 'не может быть использован';
