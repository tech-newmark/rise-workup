...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function (parameters) {
			...
			this.defaultBasketItemLogo = this.templateFolder + "/images/no-image.png";
			this.defaultStoreLogo = this.templateFolder + "/images/pickup_logo.png";
			this.defaultDeliveryLogo =
				this.templateFolder + "/images/delivery_logo.png";
			this.defaultPaySystemLogo =
				this.templateFolder + "/images/pay_system_logo.png";
			...
			this.paySystemHiddenBlockNode = BX(
				parameters.paySystemBlockId + "-hidden",
			);
			...
			if (this.totalBlockNode) {
				this.totalInfoBlockNode =
					this.totalBlockNode.querySelector(".bx-soa-cart-total");
			...
			this.options.deliveriesPerPage = parseInt(
				parameters.params.DELIVERIES_PER_PAGE,
			);
			this.options.paySystemsPerPage = parseInt(
				parameters.params.PAY_SYSTEMS_PER_PAGE,
			);
			this.options.pickUpsPerPage = parseInt(
				parameters.params.PICKUPS_PER_PAGE,
			);
		...
		sendRequest: function (action, actionData) {
			...
			if (eventArgs.action === "saveOrderAjax") {
				...
				if (form) {
					form.querySelector("input[type=hidden][name=sessid]").value =
						BX.bitrix_sessid();
				}
		...
		startLoader: function () {
			...
			if (!this.loadingScreen) {
				this.loadingScreen = new BX.PopupWindow("loader", null, {
					overlay: { backgroundColor: "rgba(0,0,0,.3)", opacity: 1 },
					events: {
						onAfterPopupShow: BX.delegate(function () {
							...
							// 	BX.create("SPAN", {
							// 		// props: { src: this.templateFolder + "/images/loader.gif" },
							// 		html: "Загрузка...",
							// 	}),
							// );
			...
			if (this.result.ORDER_PROP && this.result.ORDER_PROP.properties) {
				...
				for (i = 0; i < props.length; i++) {
					if (props[i].TYPE == "FILE") {
						prop = this.orderBlockNode.querySelector(
							'div[data-property-id-row="' + props[i].ID + '"]',
						);
		...
		checkKeyPress: function (event) {
			if (event.keyCode == 13) {
				...
				if (!send) {
					...
					if (nextAttr) {
						next = this.orderBlockNode.querySelector(
							"input[name=" + nextAttr + "]",
						);
						next && next.focus();
					}
		...
		showErrors: function (errors, scroll, showAll) {
			var errorNodes = this.orderBlockNode.querySelectorAll(
			...
			for (k in errors) {
				...
				switch (k.toUpperCase()) {
					...
					case "REGION":
						if (
							showAll ||
							this.regionBlockNode.getAttribute("data-visited") === "true"
						) {
					...
					case "PAY_SYSTEM":
						if (
							showAll ||
							this.paySystemBlockNode.getAttribute("data-visited") === "true"
						) {
					...
					case "PROPERTY":
						if (
							showAll ||
							this.propsBlockNode.getAttribute("data-visited") === "true"
						) {
		...
		checkNotifications: function () {
			var informer = this.mainErrorsNode.querySelector(
					'[data-type="informer"]',
				),
				success,
				sections,
			...
			if (informer) {
				if (
					...
					sections = this.orderBlockNode.querySelectorAll(".bx-soa-section");
					success =
						sections.length &&
						sections[sections.length - 1].getAttribute("data-visited") ==
							"true";
					className = success ? "success" : "warning";
					text = (
						success
							? this.params.MESS_SUCCESS_PRELOAD_TEXT
							: this.params.MESS_FAIL_PRELOAD_TEXT
					)
						.split("#ORDER_BUTTON#")
		...
		checkPreload: function (node) {
			...
			switch (node.id) {
				case this.regionBlockNode.id:
					status =
						this.result.LAST_ORDER_DATA &&
						this.result.LAST_ORDER_DATA.PERSON_TYPE;
					break;
				case this.paySystemBlockNode.id:
					status =
						this.result.LAST_ORDER_DATA &&
						this.result.LAST_ORDER_DATA.PAY_SYSTEM;
					break;
				case this.deliveryBlockNode.id:
					status =
						this.result.LAST_ORDER_DATA && this.result.LAST_ORDER_DATA.DELIVERY;
					break;
				case this.pickUpBlockNode.id:
					status =
						this.result.LAST_ORDER_DATA && this.result.LAST_ORDER_DATA.PICK_UP;
					break;
				default:
					status = true;
			}
		...
		checkBlockErrors: function (node) {
			...
			if ((hiddenNode = BX(node.id + "-hidden"))) {
				...
				showWarning = hiddenNode.querySelector(
					"div.alert.alert-warning.alert-show",
				);
		...
		showWarnings: function () {
			...
			if (currentDelivery && currentDelivery.CALCULATE_ERRORS) {
				...
				warningString =
					"<strong>" + this.params.MESS_DELIVERY_CALC_ERROR_TITLE + "</strong>";
						...
						case "DELIVERY":
							if (
								...
								this.showBlockWarning(
									...
									this.result.WARNING[k],
									true,
								);
								this.showBlockWarning(
									this.deliveryHiddenBlockNode,
									this.result.WARNING[k],
									true,
								);
							}
		...
		notifyAboutWarnings: function (node) {
			...
			switch (node.id) {
				...
				case this.paySystemBlockNode.id:
					this.showBlockWarning(
						this.paySystemBlockNode,
						this.result.WARNING.PAY_SYSTEM,
						true,
					);
					break;
			}
		...
		showBlockWarning: function (node, warnings, hide) {
			...
			if (errorNode) {
				...
				for (i in existedWarningNodes) {
					if (
						existedWarningNodes.hasOwnProperty(i) &&
						BX.type.isDomNode(existedWarningNodes[i])
					) {
				...
				warningNode = BX.create("DIV", {
					props: {
						className:
							"alert alert-warning" + (!!hide ? " alert-hide" : " alert-show"),
					},
					html: warnStr,
				});
		...
		showPagination: function (entity, node) {
			...
			if (pagination.pages.length > 1) {
				...
				paginationNode = BX.create("DIV", {
					props: {
						className:
							"bx-pagination" + (colorTheme ? " bx-" + colorTheme : ""),
					},
					children: [
						BX.create("DIV", {
		...
		doPagination: function (e) {
			var target = e.target || e.srcElement,
				node =
					target.tagName == "LI"
						? target
						: BX.findParent(target, { tagName: "LI" }),
		...
		showDeliveryItemsPage: function (page) {
			...
			if (selectedDelivery && selectedDelivery.ID) {
				hidden = this.deliveryBlockNode.querySelector(
			...
			deliveryItemsContainer = this.deliveryBlockNode.querySelector(
			...
			for (k = 0; k < this.deliveryPagination.currentPage.length; k++) {
				deliveryItemNode = this.createDeliveryItem(
					this.deliveryPagination.currentPage[k],
				);
		...
		showPaySystemItemsPage: function (page) {
			...
			if (selectedPaySystem && selectedPaySystem.ID) {
				hidden = this.paySystemBlockNode.querySelector(
			...
			paySystemItemsContainer = this.paySystemBlockNode.querySelector(
				".bx-soa-pp-item-container",
			);
			...
			for (k = 0; k < this.paySystemPagination.currentPage.length; k++) {
				paySystemItemNode = this.createPaySystemItem(
					this.paySystemPagination.currentPage[k],
				);
		...
		getCurrentPageItems: function (entity, page) {
			...
			if (pagination && perPage > 0) {
				...
				pagination.currentPage = pagination.pages.slice(
					pagination.pageNumber - 1,
		...
		initPropsListForLocation: function () {
			if (
				BX.saleOrderAjax &&
				this.result.ORDER_PROP &&
				this.result.ORDER_PROP.properties
			) {
				...
				for (i = 0; i < this.result.ORDER_PROP.properties.length; i++) {
					...
					if (
						curProp.TYPE == "LOCATION" &&
						curProp.MULTIPLE == "Y" &&
						curProp.IS_LOCATION != "Y"
					) {
						...
						if (
							!this.deliveryLocationInfo.city &&
							parseInt(curProp.INPUT_FIELD_LOCATION) > 0
						) {
							attrObj.altLocationPropId = parseInt(
								curProp.INPUT_FIELD_LOCATION,
							);
		...
		bindEvents: function () {
			...
			BX.addCustomEvent(
				"onDeliveryExtraServiceValueChange",
				BX.proxy(this.sendRequest, this),
			);
		},

		initFirstSection: function () {
			var firstSection = this.orderBlockNode.querySelector(
		...
		initOptions: function () {
			...
			this.propertyCollection = new BX.Sale.PropertyCollection(
			...
			if (this.result.TOTAL) {
				...
				this.options.showOrderWeight =
					total.ORDER_WEIGHT && parseFloat(total.ORDER_WEIGHT) > 0;
				this.options.showPriceWithoutDiscount =
					parseFloat(total.ORDER_PRICE) <
					parseFloat(total.PRICE_WITHOUT_DISCOUNT_VALUE);
				this.options.showDiscountPrice =
					total.DISCOUNT_PRICE && parseFloat(total.DISCOUNT_PRICE) > 0;
				this.options.showTaxList = total.TAX_LIST && total.TAX_LIST.length;
				this.options.showPayedFromInnerBudget =
					total.PAYED_FROM_ACCOUNT_FORMATED &&
					total.PAYED_FROM_ACCOUNT_FORMATED.length;
			}
		},
		...
		reachGoal: function (goal, section) {
			var counter = this.params.YM_GOALS_COUNTER || "",
				useGoals =
					this.params.USE_YM_GOALS == "Y" &&
					typeof window["yaCounter" + counter] !== "undefined",
				goalId;
		...
		getGoalId: function (goal, section) {
			...
			switch (section.id) {
				case this.basketBlockNode.id:
					goalId = isEdit
						? this.params.YM_GOALS_EDIT_BASKET
						: this.params.YM_GOALS_NEXT_BASKET;
					break;
				case this.regionBlockNode.id:
					goalId = isEdit
						? this.params.YM_GOALS_EDIT_REGION
						: this.params.YM_GOALS_NEXT_REGION;
					break;
				case this.paySystemBlockNode.id:
					goalId = isEdit
						? this.params.YM_GOALS_EDIT_PAY_SYSTEM
						: this.params.YM_GOALS_NEXT_PAY_SYSTEM;
					break;
				case this.deliveryBlockNode.id:
					goalId = isEdit
						? this.params.YM_GOALS_EDIT_DELIVERY
						: this.params.YM_GOALS_NEXT_DELIVERY;
					break;
				case this.pickUpBlockNode.id:
					goalId = isEdit
						? this.params.YM_GOALS_EDIT_PICKUP
						: this.params.YM_GOALS_NEXT_PICKUP;
					break;
				case this.propsBlockNode.id:
					goalId = isEdit
						? this.params.YM_GOALS_EDIT_PROPERTIES
						: this.params.YM_GOALS_NEXT_PROPERTIES;
					break;
			}
		...
		isPriceChanged: function (result) {
			var priceBefore =
					this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === null ||
			...
			this.options.totalPriceChanged =
				parseFloat(priceBefore) != parseFloat(priceAfter);
		},
		...
		initPagination: function () {
			...
			if (this.result.DELIVERY) {
				this.result.DELIVERY = this.getDeliverySortedArray(
					this.result.DELIVERY,
				);

				if (
					this.options.deliveriesPerPage > 0 &&
					this.result.DELIVERY.length > this.options.deliveriesPerPage
				) {
					...
					for (i = 0; i < this.result.DELIVERY.length; i++) {
						if (this.result.DELIVERY[i].CHECKED == "Y") {
							this.deliveryPagination.pageNumber = Math.ceil(
								++i / this.options.deliveriesPerPage,
							);
							break;
						}
					}
			...
			if (this.result.PAY_SYSTEM) {
				if (
					this.options.paySystemsPerPage > 0 &&
					this.result.PAY_SYSTEM.length > this.options.paySystemsPerPage
				) {
					...
					for (i = 0; i < this.result.PAY_SYSTEM.length; i++) {
						if (this.result.PAY_SYSTEM[i].CHECKED == "Y") {
							this.paySystemPagination.pageNumber = Math.ceil(
								++i / this.options.paySystemsPerPage,
							);
							break;
						}
					}
		...
		initPickUpPagination: function () {
			...
			if (this.options.pickUpsPerPage >= 0 && this.result.DELIVERY) {
				for (i = 0; i < this.result.DELIVERY.length; i++) {
					if (
						this.result.DELIVERY[i].CHECKED === "Y" &&
						this.result.DELIVERY[i].STORE_MAIN
					) {
						usePickUp = this.result.DELIVERY[i].STORE_MAIN.length > 0;
						usePickUpPagination =
							this.result.DELIVERY[i].STORE_MAIN.length >
							this.options.pickUpsPerPage;
						if (usePickUp)
							stores = this.getPickUpInfoArray(
								this.result.DELIVERY[i].STORE_MAIN,
							);
						break;
					}
			...
			if (usePickUp) {
				if (this.options.pickUpsPerPage > 0 && usePickUpPagination) {
					...
					for (i = 0; i < stores.length; i++) {
						if (
							!this.result.BUYER_STORE ||
							stores[i].ID == this.result.BUYER_STORE
						) {
							this.pickUpPagination.pageNumber = Math.ceil(
								++i / this.options.pickUpsPerPage,
							);
							break;
						}
					}
		...
		locationsCompletion: function () {
			...
			for (i in this.locations) {
				...
				locationNode = this.orderBlockNode.querySelector(
					'div[data-property-id-row="' + i + '"]',
				);
				...
				inputSearch = locationNode.querySelector(
					"input.bx-ui-sls-fake[type=text]",
				);
				...
				if (clearButton) {
					BX.bind(clearButton, "click", function (e) {
						...
						if (parent)
							locationInput = parent.querySelector(
								"input.bx-ui-sls-fake[type=text]",
							);
			...
			// 	this.editFadeRegionContent(
			// 		this.regionBlockNode.querySelector(".bx-soa-section-content"),
			// 	);

			// if (this.activeSectionId != this.propsBlockNode.id)
			// 	this.editFadePropsContent(
			// 		this.propsBlockNode.querySelector(".bx-soa-section-content"),
			// 	);
		},
		...
		fixLocationsStyle: function (section, hiddenSection) {
			...
			var regionActive =
					this.activeSectionId == section.id ? section : hiddenSection,
				locationSearchInputs,
				locationStepInputs,
				i;

			locationSearchInputs = regionActive.querySelectorAll(
				"div.bx-sls div.dropdown-block.bx-ui-sls-input-block",
			);
			locationStepInputs = regionActive.querySelectorAll(
				"div.bx-slst div.dropdown-block.bx-ui-slst-input-block",
			);
		...
		// 	) {
		// 		titleNode = section.next.querySelector(
		// 			".bx-soa-section-title-container",
		// 		);
		// 		BX.bind(titleNode, "click", BX.proxy(this.showByClick, this));
		// 		editStep = section.next.querySelector(".bx-soa-editstep");
		// 		if (editStep) editStep.style.display = "";

		// 		allSections = this.orderBlockNode.querySelectorAll(
		// 			".bx-soa-section.bx-active",
		// 		);
		...
		getNextSection: function (actionSection, skippedSection) {
			...
			var allSections = this.orderBlockNode.querySelectorAll(
					".bx-soa-section.bx-active",
				),
				nextSection,
				i;
		...
		fade: function (node, nextSection) {
			if (!node || !node.id || this.activeSectionId != node.id) return;
			...
			switch (node.id) {
				...
				case this.paySystemBlockNode.id:
					BX.remove(
						this.paySystemBlockNode.querySelector(
				...
				case this.deliveryBlockNode.id:
					BX.remove(
						this.deliveryBlockNode.querySelector(
							".alert.alert-warning.alert-hide",
						),
					);
		...
		showActualBlock: function () {
			...
			while (allSections[i]) {
				...
				if (
					!this.checkBlockErrors(allSections[i]) ||
					!this.checkPreload(allSections[i])
				) {
		...
		getBlockFooter: function (node) {
			...
			// if (
			// 	currentSection &&
			// 	currentSection.id.indexOf(firstSection.id) == "-1"
			// ) {
			// 	buttons.push(
			// 		BX.create("A", {
			// 			props: {
		...
		getNewContainer: function () {
			return BX.create("DIV", {
				props: {
					className: "bx-soa-section-content",
				},
			});
		},
		...
		// 		totalButton = this.totalBlockNode.querySelector(
		// 			".bx-soa-cart-total-button-container",
		// 		),
		// 		mobileButton = this.mobileTotalBlockNode.querySelector(
		// 			".bx-soa-cart-total-button-container",
		// 		),
		...
		shouldBeSectionVisible: function (sections, currentPosition) {
			...
			for (; currentPosition < sections.length; currentPosition++) {
				...
				if (!this.firstLoad) {
					editStepNode =
						sections[currentPosition].querySelector(".bx-soa-editstep");
		...
		changeVisibleContent: function () {
			...
			// 		this.shouldBeSectionVisible(sections, sections.length - 1),
			// 	);
			// }
		},
		...
		changeVisibleSection: function (section, state) {
			...
			if (editStep) editStep.style.display = state ? "" : "none";

			titleNode = section.querySelector(".bx-soa-section-title-container");
			if (titleNode && !state) BX.unbindAll(titleNode);
		},
		...
		editOrder: function () {
			...
			this.orderSaveBlockNode.style.display = this.result.SHOW_AUTH
				? "none"
				: "";
			// this.mobileTotalBlockNode.style.display = this.result.SHOW_AUTH
			// 	? "none"
			// 	: "";
		...
		editSection: function (section) {
			...
			if (
				this.result.SHOW_AUTH &&
				section.id != this.authBlockNode.id &&
				section.id != this.basketBlockNode.id
			)
				section.style.display = "none";
			else if (section.id != this.pickUpBlockNode.id)
				section.style.display = "";

			var active = true, //section.id == this.activeSectionId,
				titleNode = section.querySelector(".bx-soa-section-title-container"),
				editButton,
				errorContainer;
			...
			this.hasErrorSection[section.id] =
		...
		editAuthBlock: function () {
			...
			var authContent = this.authBlockNode.querySelector(
			...
			if (this.result.OK_MESSAGE && this.result.OK_MESSAGE.length) {
				this.toggleAuthForm({
					target: this.authBlockNode.querySelector("input[type=submit]"),
				});
				...
				BX.prepend(
					okMessageNode,
					this.authBlockNode.querySelector(".bx-soa-section-content"),
				);
			}
		},
			...
			new BX.PhoneAuth({
				...
				onError: function (response) {
					...
					for (var i = 0; i < response.errors.length; i++) {
						errorNode.innerHTML =
							errorNode.innerHTML +
							BX.util.htmlspecialchars(response.errors[i].message) +
							"<br>";
					}
		...
		editRegistrationForm: function (authContent) {
			...
			var authFormNodes = [];
			var showSmsConfirm =
				this.result.SMS_AUTH && this.result.SMS_AUTH.TYPE === "OK";
						...
						BX.create("LABEL", {
							...
							events: {
								change: BX.delegate(function () {
									var generated =
										this.authBlockNode.querySelector(".generated");
		...
		editSocialContent: function (authContent) {
			...
			if (this.socServiceHiddenNode === false) {
				var socServiceHiddenNode = BX("bx-soa-soc-auth-services").querySelector(
					".bx-authform-social",
				);
		...
		toggleAuthForm: function (event) {
			...
			var target = event.target || event.srcElement,
				...
				container = BX.findParent(target, {
					className: "bx-soa-section-content",
				}),
		...
		// 		basketRows = this.basketBlockNode.querySelectorAll(
		// 			".bx-soa-basket-info",
		// 		);
		// 		percent = 100;

		// 		if (basketRows.length) {
		// 			columnNodes = basketRows[0].querySelectorAll(
		...
		// 					columnNodes = basketRows[i].querySelectorAll(
		// 						".bx-soa-item-properties",
		// 					);
		...
		// 		}
		// 	} else {
		// 		columnNodes = this.basketBlockNode.querySelectorAll(
		// 			".bx-soa-item-properties",
		// 		);
		...
		editActiveBasketBlock: function (activeNodeMode) {
			...
			if (this.initialized.basket) {
				...
				basketTable = BX.create("DIV", {
					props: { className: "bx-soa-items-container" },
				});
				...
				// 	BX.create("DIV", {
				// 		props: { className: "bx-soa-table-fade ы" },
		...
		editFadeBasketBlock: function (activeNodeMode) {
			...
			if (this.initialized.basket) {
				this.basketHiddenBlockNode.appendChild(
					node.querySelector(".bx-soa-section-content"),
				);
				this.basketBlockNode.appendChild(
					BX.firstChild(this.basketHiddenBlockNode),
				);
			} else {
				newContent = this.getNewContainer();
				basketTable = BX.create("DIV", {
					props: { className: "bx-soa-item-table" },
				});
				...
				// BX.bind(
				// 	this.basketBlockNode.querySelector("div.bx-soa-table-fade")
				// 		.firstChild,
				// 	"scroll",
				// 	BX.proxy(this.basketBlockScrollCheckEvent, this),
				// );
		...
		editBasketItemsHeader: function (basketItemsNode) {
			...
			var headers = [
					BX.create("DIV", {
						props: { className: "bx-soa-item-data" },
						style: { paddingBottom: "5px" },
						children: [
							BX.create("DIV", {
								props: { className: "bx-soa-item-data-title" },
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				if (
					column.id === "DETAIL_PICTURE" &&
					!this.options.showPreviewPicInBasket
				)
					continue;

				toRight = BX.util.in_array(column.id, [
					"QUANTITY",
					"PRICE_FORMATED",
				...
				headers.push(
					BX.create("DIV", {
						props: {
							className: "bx-soa-item-data bx-soa-item-properties",
						},
						// style: { paddingBottom: "5px" },
						children: [
							BX.create("DIV", {
				...
				// 	headers.push(
				// 		BX.create("DIV", { props: { className: "bx-soa-item-nth-4p1" } }),
				// 	);
				// 	basketColumnIndex = 0;
				// }
			}
		...
		createBasketItem: function (basketItemsNode, item, index, active) {
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				otherColumns.push(
					this.createBasketItemColumn(currentColumn, item, active),
				);
			...
			if (active) {
				for (i = 0; i < this.result.GRID.HEADERS_HIDDEN.length; i++) {
					tr = this.createBasketItemHiddenColumn(
						this.result.GRID.HEADERS_HIDDEN[i],
						item,
					);
			...
			cols = [
				BX.create("DIV", {
					props: { className: "bx-soa-item-field" },
					children: [
						BX.create("DIV", {
							props: { className: "bx-soa-item-main-info" },
			...
			basketItemsNode.appendChild(
				BX.create("DIV", {
					props: {
						className: "bx-soa-item bx-soa-basket-info",
					},
					children: cols,
				}),
			);
			...
			if (hiddenColumns.length) {
				basketItemsNode.appendChild(
					BX.create("DIV", {
						props: { className: "bx-soa-item bx-soa-item-info-container" },
						children: [
							BX.create("DIV", {
								props: { className: "bx-soa-item-data" },
								children: [
		...
		showAdditionalProperties: function (event) {
			var target = event.target || event.srcElement,
				...
				parentContainer = BX.findParent(target, {
					className: "bx-soa-item bx-soa-item-info-container",
				}),
		...
		// 	var itemImg, itemTitle;

		// 	itemImg = BX.create("IMG", {
		// 		props: {
		// 			src: data.PREVIEW_PICTURE_SRC
		// 				? data.PREVIEW_PICTURE_SRC
		// 				: this.defaultBasketItemLogo,
		// 			width: "40",
		// 			height: "40",
		// 		},
		// 	});

		// 	itemTitle = BX.create("SPAN", {
		// 		html: data.NAME || "",
		// 	});

		// 	if (
		// 		this.params.HIDE_DETAIL_PAGE_URL !== "Y" &&
		// 		data.DETAIL_PAGE_URL &&
		// 		data.DETAIL_PAGE_URL.length
		// 	) {
		// 		itemImg = BX.create("A", {
		// 			props: { href: data.DETAIL_PAGE_URL },
		// 			children: [itemImg, itemTitle],
		// 		});
		// 	}
		...
		normalizeBasketItemDetailUrl: function (url) {
		...
		createBasketItemContent: function (data) {
			...
			if (
				this.params.HIDE_DETAIL_PAGE_URL !== "Y" &&
				detailPageUrl &&
				detailPageUrl.length
			) {
				titleHtml = `<a class="bx-soa-item-title" href="${this.htmlspecialcharsEx(detailPageUrl)}">${itemImg}<span>${this.htmlspecialcharsEx(itemName)}</span></a>`;
			} else {
				titleHtml = `<span class="bx-soa-item-title">${this.htmlspecialcharsEx(itemName)}</span>`;
			}

			if (this.options.showPropsInBasket && props.length) {
				props.forEach((prop) => {
					var propCode = (prop.CODE || "").toLowerCase();

					if (propCode === "favourite" || propCode === "favorite") {
						return;
					}

					propsNodes += `<div class='bx-scu-prop bx-soa-item-prop-row'><span>${this.htmlspecialcharsEx(prop.NAME || "")}</span><span>${this.htmlspecialcharsEx(prop.VALUE || "")}</span></div>`;
				});
			}

			return propsNodes.length
				? `${titleHtml}<div class="bx-scu-container">${propsNodes}</div>`
				: `${titleHtml}`;
		},
		...
		createBasketItemColumn: function (column, allData, active) {
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				...
				textNode = BX.create("DIV", {
					props: { className: "bx-soa-item-field-value" },
				}),
				logotype,
				img;
			...
			if (column.id === "PRICE_FORMATED") {
				if (parseFloat(data.DISCOUNT_PRICE) > 0) {
					textNode.appendChild(
						BX.create("STRONG", {
							props: { className: "bx-soa-item-price bx-soa-item-price--old" },
				...
				textNode.appendChild(
					BX.create("STRONG", {
						props: { className: "bx-soa-item-price" },
				...
				textNode.appendChild(
					BX.create("STRONG", {
						props: { className: "bx-soa-item-price bx-soa-item-price--all" },
						html: data.SUM,
					}),
				);
				...
				if (data["PREVIEW_TEXT_TYPE"] === "html") {
					textNode.appendChild(
						BX.create("SPAN", { html: data["PREVIEW_TEXT"] || "" }),
					);
				} else {
					textNode.appendChild(
						BX.create("SPAN", { text: data["PREVIEW_TEXT"] || "" }),
					);
				}
			} else {
				...
				if (BX.type.isArray(columnData)) {
					for (var i in columnData) {
						if (columnData.hasOwnProperty(i)) {
							if (columnData[i].type == "image")
								...
								textNode.appendChild(
									BX.create("SPAN", { html: columnData[i].value_format }),
								);
								textNode.appendChild(BX.create("BR"));
							} else if (columnData[i].value) {
								textNode.appendChild(
									BX.create("SPAN", { html: columnData[i].value }),
								);
							}
						}
					}
					...
					textNode.appendChild(
						BX.create("SPAN", { html: BX.util.htmlspecialchars(columnData) }),
					);
				}
			}
			...
			return BX.create("DIV", {
				props: {
					className: "bx-soa-item-field bx-soa-item-prop-row",
				},
				children: [
					BX.create("span", {
						props: { className: "bx-soa-item-field-title" },
						text: column.name,
					}),
					textNode,
				],
		...
		editCoupons: function (basketItemsNode) {
			var couponsList = this.getCouponsList(true),
				...
				couponsBlock = BX.create("DIV", {
					props: { className: "bx-coupon-block" },
					children: [
						BX.create("DIV", {
							props: { className: "bx-coupon-input" },
							children: [
								BX.create("INPUT", {
									props: {
						...
						BX.create("SPAN", {
							props: { className: "bx-coupon-item" },
		...
		editCouponsFade: function (basketItemsNode) {
			...
			if (couponsList.length) {
				...
				couponsBlock = BX.create("DIV", {
					...
					children: [
						BX.create("DIV", {
							props: { className: "bx-coupon-list" },
							children: [
								BX.create("DIV", {
									props: { className: "bx-coupon-item" },
				...
				basketItemsNode.appendChild(
					BX.create("DIV", {
						props: { className: "bx-coupon bx-coupon-item-fixed" },
		...
		getCouponsList: function (active) {
			...
			for (i = 0; i < this.result.COUPON_LIST.length; i++) {
				if (
					active ||
					(!active && this.result.COUPON_LIST[i].JS_STATUS == "APPLIED")
				) {
					couponsList.push(
						this.getCouponNode(
							{
								...
								desc:
									this.result.COUPON_LIST[i].JS_STATUS === "BAD"
										? BX.message("SOA_COUPON_NOT_FOUND")
										: this.result.COUPON_LIST[i].JS_STATUS === "APPLIED"
											? BX.message("SOA_COUPON_APPLIED")
											: this.result.COUPON_LIST[i].JS_STATUS === "ENTERED"
												? BX.message("SOA_COUPON_ENTERED")
												: this.result.COUPON_LIST[i].JS_CHECK_CODE,
		...
		getCouponNode: function (coupon, active) {
			...
			return BX.create("STRONG", {
				attrs: {
					...
					className: "bx-coupon-item-" + couponItem,
				},
				children: active
					? [
							couponName || "",
							BX.create("BUTTON", {
								props: {
									className: "bx-coupon-remove",
									type: "button",
									ariaLabel: "Удалить промокод",
								},
								events: {
									click: BX.delegate(function (e) {
										var target = e.target || e.srcElement,
										...
										if (coupon && coupon.getAttribute("data-coupon")) {
											this.sendRequest(
												"removeCoupon",
												coupon.getAttribute("data-coupon"),
											);
										}
									}, this),
								},
							...
							BX.create("SPAN", {
								props: {
									className:
										"bx-soa-tooltip bx-soa-tooltip-coupon bx-soa-tooltip-" +
										tooltip +
										" tooltip top",
								},
								text: couponDesc,
		...
		getCouponsLabel: function (active) {
			return BX.create("DIV", {
				props: { className: "bx-coupon-label" },
				html: active
					? this.params.MESS_USE_COUPON + ":"
		...
		addCoupon: function (coupon) {
			var couponListNodes = this.orderBlockNode.querySelectorAll(
				".bx-coupon:not(.bx-coupon-item-fixed) .bx-coupon-item",
			);

			for (var i = 0; i < couponListNodes.length; i++) {
				if (
					couponListNodes[i].querySelector(
						'[data-coupon="' + BX.util.htmlspecialchars(coupon) + '"]',
					)
				)
					break;

				couponListNodes[i].appendChild(
					this.getCouponNode({ text: coupon }, true, "bx-coupon-item-danger"),
				);
			}
		},
		...
		editRegionBlock: function (active) {
			if (
				...
				!this.regionHiddenBlockNode ||
				!this.result.PERSON_TYPE
			)
				return;
		...
		editActiveRegionBlock: function (activeNodeMode) {
			...
			if (this.initialized.region) {
				...
				regionNode = BX.create("DIV", {
					props: { className: "bx_soa_location" },
				});
				regionNodeCol = BX.create("DIV", {
					props: { className: "bx_soa_location_wrapper" },
				});
		...
		editFadeRegionBlock: function () {
			var regionContent = this.regionBlockNode.querySelector(
					".bx-soa-section-content",
				),
				newContent;
		...
		editFadeRegionContent: function (node) {
			...
			var selectedPersonType = this.getSelectedPersonType(),
				errorNode = this.regionHiddenBlockNode.querySelector(
					".alert.alert-danger",
				),
				addedHtml = "",
				props = [],
			...
			if (
				...
				this.result.PERSON_TYPE.length > 1
			) {
				addedHtml +=
					"<strong>" +
					this.params.MESS_PERSON_TYPE +
					":</strong> " +
					BX.util.htmlspecialchars(selectedPersonType.NAME);
			}
			...
			for (i in this.result.ORDER_PROP.properties) {
				if (this.result.ORDER_PROP.properties.hasOwnProperty(i)) {
					if (
						this.result.ORDER_PROP.properties[i].IS_LOCATION == "Y" &&
						this.result.ORDER_PROP.properties[i].ID ==
							this.deliveryLocationInfo.loc
					) {
						locationProperty = this.result.ORDER_PROP.properties[i];
					} else if (
						this.result.ORDER_PROP.properties[i].IS_ZIP == "Y" &&
						this.result.ORDER_PROP.properties[i].ID ==
							this.deliveryLocationInfo.zip
					) {
			...
			if (locationProperty && locationString.length)
				addedHtml +=
					"<strong>" +
					BX.util.htmlspecialchars(locationProperty.NAME) +
					":</strong> " +
					BX.util.htmlspecialchars(locationString) +
					"<br>";

			if (zipProperty && zipValue.length)
				addedHtml +=
					"<strong>" +
					BX.util.htmlspecialchars(zipProperty.NAME) +
					":</strong> " +
			...
			BX.bind(
				node.querySelector(".alert.alert-danger"),
				"click",
				BX.proxy(this.showByClick, this),
			);
			BX.bind(
				node.querySelector(".alert.alert-warning"),
				"click",
				BX.proxy(this.showByClick, this),
			);
		},
		...
		getSelectedPersonType: function () {
			...
			if (personTypeLength == 1) {
				personTypeInput = this.regionBlockNode.querySelector(
					"input[type=hidden][name=PERSON_TYPE]",
				);
				if (!personTypeInput)
					personTypeInput = this.regionHiddenBlockNode.querySelector(
						"input[type=hidden][name=PERSON_TYPE]",
					);
			} else if (personTypeLength == 2) {
				personTypeInput = this.regionBlockNode.querySelector(
					"input[type=radio][name=PERSON_TYPE]:checked",
				);
				if (!personTypeInput)
					personTypeInput = this.regionHiddenBlockNode.querySelector(
						"input[type=radio][name=PERSON_TYPE]:checked",
				...
				personTypeInput = this.regionBlockNode.querySelector(
					"select[name=PERSON_TYPE] > option:checked",
				);
				if (!personTypeInput)
					personTypeInput = this.regionHiddenBlockNode.querySelector(
						"select[name=PERSON_TYPE] > option:checked",
		...
		getDeliveryLocationInput: function (node) {
			...
			if (location && location[0] && location[0].output) {
				...
				labelHtml =
					'<label class="bx-soa-label" for="soa-property-' +
					parseInt(locationId) +
					'">' +
					...
					(currentProperty.DESCRIPTION.length
						? " <small>(" +
							BX.util.htmlspecialchars(currentProperty.DESCRIPTION) +
							")</small>"
						: "") +
					"</label>";
			...
			if (location && location[0]) {
				node.appendChild(
					BX.create("DIV", {
						props: { className: "bx-soa-reference alert alert-warning" },
			...
			if (altProperty) {
				...
				input = BX.create("INPUT", {
					props: {
						...
						autocomplete: "city",
						className: "form-control bx-soa-customer-input main-input",
		...
		getLocationString: function (node) {
			...
			if (
				locationInputNode &&
				locationInputNode.value &&
				locationInputNode.value.length
			)
				locationString = locationInputNode.value;
			else {
				locationSteps = node.querySelectorAll(
		...
		getPersonTypeControl: function (node) {
			...
			this.result.PERSON_TYPE = this.getPersonTypeSortedArray(
				this.result.PERSON_TYPE,
			);
			...
			if (personTypesCount > 2) {
				...
				for (i in this.result.PERSON_TYPE) {
					if (this.result.PERSON_TYPE.hasOwnProperty(i)) {
						...
						if (delimiter) node.appendChild(BX.create("BR"));
		...
		getProfilesControl: function (node) {
			var profilesLength = BX.util.object_keys(
			...
			if (profilesLength) {
				if (
					...
					node.appendChild(
						BX.create("DIV", {
							props: {
								className: "form-group bx-soa-location-input-container",
							},
		...
		createPaySystemItem: function (item) {
			...
			logoNode = BX.create("DIV", {
				props: { className: "bx-soa-pp-company-image" },
			});
		...
		editPaySystemInfo: function (paySystemNode) {
			if (
				!this.result.PAY_SYSTEM ||
				(this.result.PAY_SYSTEM.length == 0 &&
					this.result.PAY_FROM_ACCOUNT != "Y")
			)
				return;
			...
			if (currentPaySystem) {
				...
				if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == "Y") {
					subTitle = BX.create("DIV", {
						props: { className: "bx-soa-pp-company-name" },
		...
		getInnerPaySystem: function () {
			...
			var accountOnly =
					this.params.ONLY_FULL_PAY_FROM_ACCOUNT &&
					this.params.ONLY_FULL_PAY_FROM_ACCOUNT == "Y",
				isSelected =
					this.result.PAY_CURRENT_ACCOUNT &&
					this.result.PAY_CURRENT_ACCOUNT == "Y",
			...
			return BX.create("DIV", {
				props: {
					className: "bx-soa-pp-inner-ps" + (isSelected ? " bx-selected" : ""),
				},
		...
		editFadePaySystemContent: function (node) {
			var selectedPaySystem = this.getSelectedPaySystem(),
				errorNode = this.paySystemHiddenBlockNode.querySelector(
					"div.alert.alert-danger",
				),
				warningNode = this.paySystemHiddenBlockNode.querySelector(
					"div.alert.alert-warning.alert-show",
				),
				addedHtml = "",
				logotype,
			...
			if (this.isSelectedInnerPayment()) {
				...
				addedHtml +=
					"<strong>" + this.result.INNER_PAY_SYSTEM.NAME + "</strong><br>";
				addedHtml += "</div>";
			}
			...
			if (selectedPaySystem && selectedPaySystem.NAME) {
				...
				addedHtml +=
					"<strong>" +
					BX.util.htmlspecialchars(selectedPaySystem.NAME) +
					"</strong>";
				addedHtml += "</div>";
			}
			...
			if (!addedHtml.length)
				addedHtml =
					"<strong>" + BX.message("SOA_PS_SELECT_ERROR") + "</strong>";
		...
		getSelectedPaySystem: function () {
			var paySystemCheckbox = this.paySystemBlockNode.querySelector(
					"input[type=checkbox][name=PAY_SYSTEM_ID]:checked",
				),
		...
		isSelectedInnerPayment: function () {
			var innerPaySystemCheckbox = this.paySystemBlockNode.querySelector(
				"input[type=checkbox][name=PAY_CURRENT_ACCOUNT]",
			);

			if (!innerPaySystemCheckbox)
				innerPaySystemCheckbox = this.paySystemHiddenBlockNode.querySelector(
					"input[type=checkbox][name=PAY_CURRENT_ACCOUNT]",
				);
		...
		selectPaySystem: function (event) {
			...
			var target = event.target || event.srcElement,
				innerPaySystemSection = this.paySystemBlockNode.querySelector(
					"div.bx-soa-pp-inner-ps",
				),
				innerPaySystemCheckbox = this.paySystemBlockNode.querySelector(
					"input[type=checkbox][name=PAY_CURRENT_ACCOUNT]",
				),
				fullPayFromInnerPaySystem =
					this.result.TOTAL &&
					parseFloat(this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY) === 0;
			...
			if (innerPsAction) {
				...
				if (
					innerPaySystemCheckbox &&
					innerPaySystemCheckbox.checked &&
					fullPayFromInnerPaySystem
				) {
					...
					if (selectedSection) {
						...
						selectedSection.querySelector("input[type=checkbox]").checked =
							false;
					}
				}
			}
		...
		editDeliveryInfo: function (deliveryNode) {
			...
			var deliveryInfoContainer = BX.create("DIV", {
					props: { className: "bx-soa-pp-desc-container" },
				}),
				currentDelivery,
				logotype,
				name,
			...
			if (currentDelivery.PRICE > 0) {
				price = BX.create("LI", {
					children: [
						BX.create("SPAN", {
							props: { className: "bx-soa-pp-list-name" },
							html: this.params.MESS_PRICE + ":",
						}),
						BX.create("SPAN", {
							props: { className: "bx-soa-pp-list-value" },
			...
			deliveryInfoContainer.appendChild(
				BX.create("DIV", {
					...
					children: [
						...
						clear,
						extraServicesNode,
						currentDelivery.PRICE > 0 ? infoList : null,
					],
				}),
			);
		...
		getDeliveryPriceNodes: function (delivery) {
			...
			if (
				...
				parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) !=
					parseFloat(delivery.PRICE)
			) {
				if (
					parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) >
					parseFloat(delivery.PRICE)
					...
					priceNodesArray = [
						...
						BX.create("SPAN", {
							props: {
								className:
									"bx-soa-cart-total-line-price bx-soa-cart-total-line-price--old",
							},
		...
		editFadeDeliveryBlock: function () {
			var deliveryContent = this.deliveryBlockNode.querySelector(
					".bx-soa-section-content",
				),
				newContent;
		...
		createDeliveryItem: function (item) {
			...
			// 	labelNodes.push(
			// 		BX.create("SMALL", {
			// 			props: { className: "bx-soa-pp-delivery-cost 1" },
			// 			html:
			// 				typeof item.DELIVERY_DISCOUNT_PRICE !== "undefined"
			...
			// 	labelNodes.push(
			// 		BX.create("SMALL", {
			// 			props: { className: "bx-soa-pp-delivery-cost 2" },
			// 			html:
			// 				typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== "undefined"
			// 					? "Стоимость: " +
			// 						deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED
			// 					: "Стоимость: " + deliveryCached.PRICE_FORMATED,
			// 		}),
			// 	);
			// }
			...
			label = BX.create("DIV", {
				props: {
					className:
						...
						(item.CALCULATE_ERRORS ||
						(deliveryCached && deliveryCached.CALCULATE_ERRORS)
							? " bx-bd-waring"
							: ""),
				},
				children: labelNodes,
		...
		editFadeDeliveryContent: function (node) {
			var selectedDelivery = this.getSelectedDelivery(),
				name =
					this.params.SHOW_DELIVERY_PARENT_NAMES != "N"
						...
						: selectedDelivery.OWN_NAME,
				errorNode = this.deliveryHiddenBlockNode.querySelector(
					"div.alert.alert-danger",
				),
				warningNode = this.deliveryHiddenBlockNode.querySelector(
					"div.alert.alert-warning.alert-show",
				),
				extraService,
				logotype,
			...
			if (selectedDelivery && selectedDelivery.NAME) {
				...
				if (
					this.params.DELIVERY_FADE_EXTRA_SERVICES == "Y" &&
					BX.util.object_keys(selectedDelivery.EXTRA_SERVICES).length
				) {
					...
					for (i in selectedDelivery.EXTRA_SERVICES) {
						if (selectedDelivery.EXTRA_SERVICES.hasOwnProperty(i)) {
							...
							if (
								extraService.value &&
								extraService.value != "N" &&
								extraService.canUserEditValue
							) {
								arNodes.push(BX.create("BR"));
								arNodes.push(
									BX.create("STRONG", { text: extraService.name + ": " }),
								);
				...
				node.appendChild(
					BX.create("STRONG", {
						text: BX.message("SOA_DELIVERY_SELECT_ERROR"),
					}),
				);
		...
		selectDelivery: function (event) {
			...
			var target = event.target || event.srcElement,
				actionSection = BX.hasClass(target, "bx-soa-pp-company")
					? target
					: BX.findParent(target, { className: "bx-soa-pp-company" }),
				selectedSection = this.deliveryBlockNode.querySelector(
					".bx-soa-pp-company.bx-selected",
				),
				actionInput,
				selectedInput;
		...
		editFadePickUpBlock: function () {
			var pickUpContent = this.pickUpBlockNode.querySelector(
					".bx-soa-section-content",
				),
				newContent;
		...
		editFadePickUpContent: function (pickUpContainer) {
			...
			if (selectedPickUp) {
				if (this.params.SHOW_STORES_IMAGES == "Y") {
					...
					html +=
						'<img src="' + imgSrc + '" class="bx-soa-pickup-preview-img">';
				}

				html +=
					"<strong>" +
					BX.util.htmlspecialchars(selectedPickUp.TITLE) +
					"</strong>";
				...
				if (this.params.SHOW_STORES_IMAGES == "Y") {
					BX.bind(
						pickUpContainer.querySelector(".bx-soa-pickup-preview-img"),
						"click",
						BX.delegate(function (e) {
		...
		geoLocationSuccessCallback: function (result) {
			...
			if (
				activeStores &&
				activeStores.length >= this.options.pickUpMap.minToShowNearestBlock
			) {
		...
		getPropertyMapData: function () {
			...
			if (
				...
				this.locations[locationId][0] &&
				this.locations[locationId][0].coordinates
			) {
		...
		editPickUpList: function (isNew) {
			if (
				!this.pickUpPagination.currentPage ||
				!this.pickUpPagination.currentPage.length
			)
				return;
			...
			var pickUpList = BX.create("DIV", {
					props: { className: "bx-soa-pickup-list main" },
				}),
			...
			recommendList = this.pickUpBlockNode.querySelector(
				".bx-soa-pickup-list.recommend",
			);
			if (!recommendList)
				recommendList = this.pickUpHiddenBlockNode.querySelector(
					".bx-soa-pickup-list.recommend",
				);

			if (
				!recommendList ||
				!recommendList.querySelector(".bx-soa-pickup-list-item.bx-selected")
			) {
			...
			for (i = 0; i < this.pickUpPagination.currentPage.length; i++) {
				...
				if (
					currentStore.ID == selectedStore ||
					parseInt(selectedStore) == 0 ||
					!found
				) {
			...
			if (!!isNew) {
				container =
					this.pickUpHiddenBlockNode.querySelector(".bx_soa_pickup>div");
				if (!container)
				...
				container = this.pickUpBlockNode.querySelector(
		...
		getStoreInfoHtml: function (currentStore) {
			...
			if (currentStore.DESCRIPTION)
				html +=
					BX.message("SOA_PICKUP_DESC") +
					": " +
					BX.util.htmlspecialchars(currentStore.DESCRIPTION) +
					"<br>";
		...
		editPickUpRecommendList: function (geoLocation) {
			...
			var recommendList = BX.create("DIV", {
					props: { className: "bx-soa-pickup-list recommend" },
				}),
				buyerStoreInput = BX("BUYER_STORE"),
				selectedDelivery = this.getSelectedDelivery();
			...
			for (i = 0; i < recommendedStoreIds.length; i++) {
				...
				if (
					i === 0 &&
					parseInt(selectedDelivery.ID) !== this.lastSelectedDelivery
				) {
				...
				if (selectedDelivery.STORE_MAIN) {
					selectedDelivery.STORE_MAIN.splice(
						selectedDelivery.STORE_MAIN.indexOf(currentStoreId),
						1,
					);
				}
			}
		...
		getDeliverySortedArray: function (objDelivery) {
			...
			for (k in objDelivery) {
				if (objDelivery.hasOwnProperty(k)) {
					if (
						this.params.SHOW_NOT_CALCULATED_DELIVERIES === "L" &&
						objDelivery[k].CALCULATE_ERRORS
					) {
		...
		editActivePropsBlock: function (activeNodeMode) {
			...
			if (this.initialized.props) {
				...
				this.maps &&
					setTimeout(BX.proxy(this.maps.propsMapFocusWaiter, this.maps), 200);
			} else {
				...
				if (
					...
					for (i = 0; i < this.params.SHOW_MAP_FOR_DELIVERIES.length; i++) {
						if (
							...
							parseInt(this.params.SHOW_MAP_FOR_DELIVERIES[i])
						) {
							showPropMap = true;
							break;
						}
		...
		editFadePropsBlock: function () {
			var propsContent = this.propsBlockNode.querySelector(
					".bx-soa-section-content",
				),
				newContent;
		...
		editFadePropsContent: function (node) {
			...
			if (!props || props.length === 0) {
				node.innerHTML +=
					"<strong>" + BX.message("SOA_ORDER_PROPS") + "</strong>";
			} else {
		...
		editPropsItems: function (propsNode) {
			...
			var propsItemsContainer = BX.create("DIV", {
					props: { className: "bx-soa-customer" },
				}),
				group,
				property,
			...
			if (!propsItemsContainer)
				propsItemsContainer =
					this.propsBlockNode.querySelector(".bx-soa-customer");
		...
		getPropertyRowNode: function (property, propsItemsContainer, disabled) {
			...
			if (disabled) {
				...
				if (property.isRequired())
					textHtml += '<small class="bx-authform-starrequired">*</small>';
				...
				if (
					...
					textHtml +=
						" <small>(" + BX.util.htmlspecialchars(propertyDesc) + ")</small>";
		...
		insertLocationProperty: function (property, propsItemNode, disabled) {
			...
			if (property.getId() in this.locations) {
				if (disabled) {
					propRow = this.propsHiddenBlockNode.querySelector(
						'[data-property-id-row="' + property.getId() + '"]',
					);
					if (propRow) {
					...
					propContainer = BX.create("DIV", {
						props: { className: "soa-property-container" },
					});
					...
					for (i = 0; i < propRow.length; i++) {
						...
						if (property.isMultiple())
							insertedLoc.style.marginBottom =
								this.locationsTemplate == "search" ? "5px" : "20px";
		...
		addLocationProperty: function (e) {
			...
			if (BX.hasClass(lastProp, "bx-soa-loc")) {
				if (this.locationsTemplate == "search") {
					input = lastProp.querySelector(
						"input[type=text][class=dropdown-field]",
					);
					if (input)
						index =
							parseInt(
			...
			if (this.cleanLocations[propId]) {
				insertedLoc = BX.create("DIV", {
					props: { className: "bx-soa-loc" },
					style: {
						marginBottom: this.locationsTemplate == "search" ? "5px" : "20px",
					},
		...
		insertFileProperty: function (property, propsItemNode, disabled) {
			...
			if (disabled) {
				...
				saved = this.savedFilesBlockNode.querySelector(
					'div[data-property-id-row="' + property.getId() + '"]',
				);
				if (saved)
			...
			if (disabled) {
				...
				if (prop) {
					...
					for (i = 0; i < inputs.length; i += 2)
						values.push(
							inputs[i].checked ? BX.message("SOA_YES") : BX.message("SOA_NO"),
						);
		...
		alterProperty: function (settings, propContainer) {
			...
			for (i = 0; i < inputs.length; i++) {
				...
				BX.addClass(
					inputs[i],
					"form-control bx-soa-customer-input main-input --iconed-field",
				);
			}
			...
			for (i = 0; i < inputs.length; i++) {
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], "form-control main-textarea --iconed-field");
			}
			...
			if (settings.TYPE == "FILE") {
				...
				for (i = 0; i < fileTitles.length; i++) {
					BX.bind(fileTitles[i], "click", function (e) {
						var target = e.target || e.srcElement,
							fileInput =
								target && target.nextSibling && target.nextSibling.nextSibling;
				...
				BX.bind(
					...
					BX.delegate(function (e) {
						...
						if (add.length && add[add.length - 2]) {
							...
							BX.addClass(add[add.length - 2], "main-btn");
						}

						del && BX.remove(del);
						if (textInputs.length) {
							textInputs[textInputs.length - 1].placeholder =
								settings.DESCRIPTION;
							BX.addClass(
								textInputs[textInputs.length - 1],
								"form-control bx-soa-customer-input main-input",
							);
							if (settings.TYPE == "DATE")
								this.alterDateProperty(
									settings,
									textInputs[textInputs.length - 1],
								);
						...
						if (textAreas.length) {
							textAreas[textAreas.length - 1].placeholder =
								settings.DESCRIPTION;
							BX.addClass(textAreas[textAreas.length - 1], "form-control");
						}
		...
		isValidRegionBlock: function () {
			...
			var regionProps = this.orderBlockNode.querySelectorAll(
					".bx-soa-location-input-container[data-property-id-row]",
				),
				regionErrors = [],
				id,
				arProperty,
		...
		// 		quickLocation = targetNode.parentNode.querySelector(
		// 			"div.quick-locations",
		// 		);
		...
		validateString: function (input, arProperty, fieldName) {
			...
			if (arProperty.REQUIRED === "Y" && value.length === 0) {
				errors.push(
					fieldName
						? BX.Loc.getMessage("SOA_REQUIRED_FIELD_WITH_NAME", {
								"#NAME#": name,
							})
			...
			if (value.length) {
				if (arProperty.MINLENGTH && arProperty.MINLENGTH > value.length) {
					errors.push(
						BX.Loc.getMessage("SOA_MIN_LENGTH_FIELD_WITH_NAME", {
							"#NAME#": name,
				...
				if (arProperty.MAXLENGTH && arProperty.MAXLENGTH < value.length) {
					errors.push(
						BX.Loc.getMessage("SOA_MAX_LENGTH_FIELD_WITH_NAME", {
							"#NAME#": name,
				...
				if (
					value.length > 0 &&
					arProperty.PATTERN &&
					arProperty.PATTERN.length
				) {
					re = new RegExp(arProperty.PATTERN);
					if (!re.test(value)) {
						errors.push(
							fieldName
								? BX.Loc.getMessage("SOA_INVALID_PATTERN_FIELD_WITH_NAME", {
										"#NAME#": name,
									})
		...
		validateLocation: function (input, arProperty, fieldName) {
			...
			if (
				...
				(value.length === 0 || value === BX.Loc.getMessage("SOA_NOT_SPECIFIED"))
			) {
				errors.push(
					fieldName
						? BX.Loc.getMessage("SOA_REQUIRED_FIELD_WITH_NAME", {
		...
		validateNumber: function (input, arProperty, fieldName) {
			...
			if (value.length) {
				if (!/[0-9]|\./.test(value)) {
					errors.push(
						fieldName
							? BX.Loc.getMessage("SOA_NOT_NUMERIC_FIELD_WITH_TITLE", {
									"#NAME#": name,
								})
				...
				if (arProperty.MIN && parseFloat(arProperty.MIN) > parseFloat(value)) {
					errors.push(
						BX.Loc.getMessage("SOA_MIN_VALUE_FIELD_WITH_NAME", {
							"#NAME#": name,
				...
				if (arProperty.MAX && parseFloat(arProperty.MAX) < parseFloat(value)) {
					errors.push(
						BX.Loc.getMessage("SOA_MAX_VALUE_FIELD_WITH_NAME", {
							"#NAME#": name,
				...
				if (arProperty.STEP && parseFloat(arProperty.STEP) > 0) {
					var num = Math.abs(
						parseFloat(value) -
							(arProperty.MIN && parseFloat(arProperty.MIN) > 0
								? parseFloat(arProperty.MIN)
								: 0),
					);
					...
					if (del !== parseInt(del)) {
						errors.push(
							fieldName
								? BX.Loc.getMessage("SOA_NUM_STEP_FIELD_WITH_NAME", {
										"#NAME#": name,
										"#STEP#": arProperty.STEP,
									})
								: BX.Loc.getMessage("SOA_NUM_STEP_FIELD", {
										"#STEP#": arProperty.STEP,
									}),
						);
					}
				...
				for (var file of files) {
					...
					if (
						...
						(fileExtension.length === 0 ||
							arProperty.ACCEPT.indexOf(fileExtension) === "-1")
					) {
						errors.push(
							BX.Loc.getMessage("SOA_BAD_EXTENSION_NAME_AND_ACCEPT", {
								"#NAME#": fileName,
					...
					if (file.size > parseInt(arProperty.MAXSIZE)) {
						errors.push(
							BX.Loc.getMessage("SOA_MAX_SIZE_NAME_AND_SIZE", {
								"#NAME#": fileName,
		...
		editPropsComment: function (propsNode) {
			...
			input = BX.create("TEXTAREA", {
				...
				text: this.result.ORDER_DESCRIPTION
					? this.result.ORDER_DESCRIPTION
					: "",
			});
		...
		editTotalBlock: function () {
			...
			if (this.options.showPriceWithoutDiscount) {
				priceHtml +=
					'<br><span class="bx-soa-cart-total-line-price bx-soa-cart-total-line-price--old">' +
					total.PRICE_WITHOUT_DISCOUNT +
					"</span>";
			}

			this.totalInfoBlockNode.appendChild(
				this.createTotalUnit(BX.message("SOA_SUM_SUMMARY"), priceHtml, params),
			);

			if (this.options.showOrderWeight) {
				this.totalInfoBlockNode.appendChild(
					this.createTotalUnit(
						BX.message("SOA_SUM_WEIGHT_SUM"),
						total.ORDER_WEIGHT_FORMATED,
					),
				);
			}
					...
					this.totalInfoBlockNode.appendChild(
						this.createTotalUnit(
							total.TAX_LIST[i].NAME +
								(!!total.TAX_LIST[i].VALUE_FORMATED
									? " " + total.TAX_LIST[i].VALUE_FORMATED
									: "") +
								":",
							valFormatted,
			...
			deliveryError =
				...
				curDelivery.CALCULATE_ERRORS.length;
			...
			if (deliveryError) {
				...
				if (
					...
					parseFloat(curDelivery.PRICE) >
						parseFloat(curDelivery.DELIVERY_DISCOUNT_PRICE)
				) {
					deliveryValue +=
						'<br><span class="bx-soa-item-price-old">' +
						curDelivery.PRICE_FORMATED +
						"</span>";
				}
			}
			...
			if (this.options.showDiscountPrice) {
				...
				this.totalInfoBlockNode.appendChild(
					this.createTotalUnit(discText + ":", total.DISCOUNT_PRICE_FORMATED, {
						highlighted: true,
					}),
				);
			}
			...
			if (this.options.showPayedFromInnerBudget) {
				...
				this.totalInfoBlockNode.appendChild(
					this.createTotalUnit(
						BX.message("SOA_SUM_LEFT_TO_PAY"),
			...
			if (
				parseFloat(total.PAY_SYSTEM_PRICE) >= 0 &&
				this.result.DELIVERY.length
			) {
				this.totalInfoBlockNode.appendChild(
					this.createTotalUnit(
						BX.message("SOA_PAYSYSTEM_PRICE"),
						"~" + total.PAY_SYSTEM_PRICE_FORMATTED,
					),
				);
			}

			// Скрываю доп кнопку оформления заказа
			// if (!this.result.SHOW_AUTH) {
			// 	this.totalInfoBlockNode.appendChild(
			// 		BX.create("DIV", {
			// 			props: {
		...
		// 	this.mobileTotalBlockNode.appendChild(
		// 		this.totalInfoBlockNode.cloneNode(true),
		// 	);
		// 	BX.bind(
				...
				children: [
					...
					BX.create("SPAN", {
						props: {
							// className:
							// 	"bx-soa-cart-d" +
							// 	(!!params.total && this.options.totalPriceChanged
		...
		// 	var scrollableNodes = this.orderBlockNode.querySelectorAll(
		// 			"div.bx-soa-table-fade",
		// 		),
		// 		parentNode,
		// 		parentWidth,
		// 		tableNode,
		...
		// 			scrollRight =
		// 				scrollNode.scrollWidth - (scrollLeft + scrollNode.clientWidth);
		...
		// 			BX.removeClass(
		// 				parentNode,
		// 				"bx-soa-table-fade-left bx-soa-table-fade-right",
		// 			);
		// 	}
		// },
		...
		totalBlockResizeCheck: function () {
			...
			if (BX.hasClass(this.totalInfoBlockNode, "bx-soa-cart-total-fixed"))
				this.totalInfoBlockNode.style.width =
					this.totalGhostBlockNode.offsetWidth + "px";
		},

		totalBlockFixFont: function () {
			var totalNode = this.totalInfoBlockNode.querySelector(
			...
			if (this.params.SHOW_TOTAL_ORDER_BUTTON == "Y") {
				buttonNode = this.totalInfoBlockNode.querySelector(
					".bx-soa-cart-total-button-container",
				);
				if (buttonNode) {
			...
			for (i in this.result.GRID.ROWS) {
				if (this.result.GRID.ROWS.hasOwnProperty(i)) {
					...
					products.push({
						...
						brand: (item.data[this.params.BRAND_PROPERTY + "_VALUE"] || "")
							.split(", ")
							.join("/"),
		...
		initUserConsent: function () {
			BX.ready(
				BX.delegate(function () {
					var control =
						BX.UserConsent && BX.UserConsent.load(this.orderBlockNode);
					if (control) {
						BX.addCustomEvent(
							control,
							BX.UserConsent.events.save,
							BX.proxy(this.doSaveAction, this),
						);
						BX.addCustomEvent(
							control,
							BX.UserConsent.events.refused,
							BX.proxy(this.disallowOrderSave, this),
						);
					}
				}, this),
			);
