	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function (parameters) {
			...
			this.defaultBasketItemLogo =
				this.templateFolder + "/images/product_logo.png";
		...
		checkNotifications: function () {
			...
			if (informer) {
				if (
					...
					sections = this.orderBlockNode.querySelectorAll(
		...
		getNewContainer: function (notFluid) {
			return BX.create("DIV", {
				props: {
					className:
						"bx-soa-section-content" + (!!notFluid ? "" : " container-fluid"),
				},
			});
		},
		...
		editSocialContent: function (authContent) {
			...
			authContent.appendChild(
				BX.create("DIV", { props: { className: "col-md-6" }, children: nodes }),
			);
		},
		...
		showAdditionalProperties: function (event) {
			var target = event.target || event.srcElement,
				...
				parentContainer = BX.findParent(target, {
					className: "bx-soa-item-tr bx-soa-item-info-container",
				}),
		...
		createBasketItemContent: function (data) {
			...
			if (
				...
				titleHtml =
					'<a href="' + data.DETAIL_PAGE_URL + '">' + titleHtml + "</a>";
			}
		...
		editActiveRegionBlock: function (activeNodeMode) {
			...
			if (this.initialized.region) {
				...
				regionNode = BX.create("DIV", {
					props: { className: "bx_soa_location row" },
				});
		...
		editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create("DIV", {
					props: { className: "col-sm-7 bx-soa-pp-item-container" },
				}),
				paySystemItemNode,
				i;
		...
		editPaySystemInfo: function (paySystemNode) {
			...
			var paySystemInfoContainer = BX.create("DIV", {
					props: {
						className:
							(this.result.PAY_SYSTEM.length == 0 ? "col-sm-12" : "col-sm-5") +
		...
		editDeliveryInfo: function (deliveryNode) {
			...
			var deliveryInfoContainer = BX.create("DIV", {
					props: { className: "col-sm-5 bx-soa-pp-desc-container" },
				}),
				currentDelivery,
				logotype,
				name,
		...
		editPickUpList: function (isNew) {
			...
			if (!!isNew) {
				container = this.pickUpHiddenBlockNode.querySelector(
					".bx_soa_pickup>.col-xs-12",
				);
				if (!container)
		...
		editPropsItems: function (propsNode) {
			...
			if (!propsItemsContainer)
				propsItemsContainer = this.propsBlockNode.querySelector(
