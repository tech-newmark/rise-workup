BX.saleOrderAjax = {
	...
	initDeferredControl: function () {
		...
		for (k in this.properties) {
			...
			if (this.properties[k].isZip) {
				row = this.controls.scope.querySelector(
					'[data-property-id-row="' + k + '"]',
				);
								...
								if (
									...
									ctx.getLocationsByZip(
										...
										function (locationsData) {
											ctx.properties[locPropId].control.setValueByLocationIds(
												locationsData,
											);
										},
										function () {
				...
				if (typeof this.properties[k].control != "undefined") {
					...
					if (typeof this.properties[k].altLocationPropId != "undefined") {
						if (code == "sls") // for sale.location.selector.search
						{
							...
							control.replaceTemplate(
								"nothing-found",
								this.options.messages.notFoundPrompt,
							);
						}
							...
							(function (k, control) {
								...
								control.bindEvent(
									...
									function (adapter) {
										...
										if (
											parentValue == this.getOption("rootNodeValue") ||
											!this.checkCanSelectItem(parentValue)
										)
											return;
										...
										if (
											typeof controlInApater.vars.cache.nodes["other"] ==
											"undefined"
										) {
								...
								townInputFlag = BX(
									"LOCATION_ALT_PROP_DISPLAY_MANUAL[" + parseInt(k) + "]",
								);
								...
								if (
									BX.type.isDomNode(townInputFlag) &&
									townInputFlag.value == "1"
								) {
									// a little hack: set "other location" text display
									adapter = control.getAdapterAtPosition(
										control.getStackSize() - 1,
									);

									if (typeof adapter != "undefined" && adapter !== null)
										adapter.setValuePair(
											"other",
											ctx.options.messages.otherLocation,
										);
								}
							})(k, control);
						}
	...
	checkMode: function (propId, mode) {
		...
		if (mode == "altLocationChoosen") {
			if (this.checkAbility(propId, "canHaveAltLocation")) {
				var input = this.getInputByPropId(
					this.properties[propId].altLocationPropId,
				);
	...
	checkAbility: function (propId, ability) {
		...
		if (
			typeof this.properties[propId].abilities != "undefined" &&
			this.properties[propId].abilities[ability]
		)
			return true;
	...
	getRowByPropId: function (propId) {
		...
		var row = this.controls.scope.querySelector(
			'[data-property-id-row="' + propId + '"]',
		);
		if (BX.type.isElementNode(row)) {
	...
	submitFormProxy: function (item, control) {
		...
		for (var k in this.properties) {
			if (
				typeof this.properties[k].control != "undefined" &&
				this.properties[k].control == control
			) {
				propId = k;
				break;
			}
