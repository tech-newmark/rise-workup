...
if ($arResult['ALLOW_ANONYMOUS'] == 'Y' && $_REQUEST['authorize'] <> 'YES' && $_REQUEST['register'] <> 'YES'): ?>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data-table">
		<thead>
			<tr>
				<td colspan="2"><?php echo GetMessage('subscr_title_auth2') ?></td>
			</tr>
		</thead>
		<tr valign="top">
			<td width="40%">
...
<?php elseif ($arResult['ALLOW_ANONYMOUS'] == 'N' || $_REQUEST['authorize'] == 'YES' || $_REQUEST['register'] == 'YES'): ?>
	<form action="<?= $arResult['FORM_ACTION'] ?>" method="post">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data-table">
			<thead>
				<tr>
					<td colspan="2"><?php echo GetMessage('adm_auth_exist') ?></td>
				</tr>
			</thead>
			<tr valign="top">
				<td width="40%">
					<p><?php echo GetMessage('adm_auth_login') ?><span class="starrequired">*</span><br />
						<input type="text" name="LOGIN" value="<?php echo $arResult['REQUEST']['LOGIN'] ?>" size="20" />
					</p>
					<p><?php echo GetMessage('adm_auth_pass') ?><span class="starrequired">*</span><br />
						<input type="password" name="PASSWORD" size="20" value="<?php echo $arResult['REQUEST']['PASSWORD'] ?>" />
					</p>
				</td>
				<td width="60%">
			...
			<tfoot>
				<tr>
					<td colspan="2"><input type="submit" name="Save" value="<?php echo GetMessage('adm_auth_butt') ?>" /></td>
				</tr>
			</tfoot>
		</table>
	...
	?>
		<form action="<?= $arResult['FORM_ACTION'] ?>" method="post">
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data-table">
				<thead>
					<tr>
						<td colspan="2"><?php echo GetMessage('adm_reg_new') ?></td>
					</tr>
				</thead>
				<tr valign="top">
					<td width="40%">
						<p><?php echo GetMessage('adm_reg_login') ?><span class="starrequired">*</span><br />
							<input type="text" name="NEW_LOGIN" value="<?php echo $arResult['REQUEST']['NEW_LOGIN'] ?>" size="20" />
						</p>
						<p><?php echo GetMessage('adm_reg_pass') ?><span class="starrequired">*</span><br />
							<input type="password" name="NEW_PASSWORD" size="20" value="<?php echo $arResult['REQUEST']['NEW_PASSWORD'] ?>" />
						</p>
						<p><?php echo GetMessage('adm_reg_pass_conf') ?><span class="starrequired">*</span><br />
							<input type="password" name="CONFIRM_PASSWORD" size="20" value="<?php echo $arResult['REQUEST']['CONFIRM_PASSWORD'] ?>" />
						</p>
				...
				<tfoot>
					<tr>
						<td colspan="2"><input type="submit" name="Save" value="<?php echo GetMessage('adm_reg_butt') ?>" /></td>
					</tr>
				</tfoot>
			</table>
			<?php foreach ($arResult['RUBRICS'] as $itemValue): ?>
